...
export default function Home_Type() {
    ...
    return (
        APITP.filter((e) => e.pro_status == "true").length !== 0 ?
        <>
            <div className='relative flex justify-center w-screen h-full'>
                <div className='w-[80%] relative py-[10%] md:flex grid gap-6 flex justify-center'>
                        ...
                        index == 1 ?
                            APITT.filter((e) => e.v1typeId?.pro_type == item.pro_type).slice(0, 1).map((img) => (
                                <div className='relative lg:w-[50vh] md:w-[30vh] lg:min-h-[55vh] md:min-h-[35vh] overflow-hidden rounded'>
                                    <img src={data.IMG + img.v2image} className='relative z-10 object-cover w-full h-full scale-100 hover:scale-110 ease-in duration-200' />
